...
if (strlen(session_id()) < 1)
	session_start();
require_once "../modelos/Clientef.php";

$clientef= new Clientef();

$idclientefinal=isset($_POST["idclientefinal"])? limpiarCadena($_POST["idclientefinal"]):"";
...
switch ($_GET["op"]){
	case 'guardaryeditar':	
		...
		if (!file_exists($_FILES['imagen']['tmp_name']) || !is_uploaded_file($_FILES['imagen']['tmp_name']))
		{
			...
			if($_FILES['imagen']['type'] == "image/jpg" || $_FILES['imagen']['type'] == "image/jpeg" || $_FILES['imagen']['type'] == "image/png")
			{
				...
				move_uploaded_file($_FILES["imagen"]["tmp_name"], "../files/clientefinal/" . $imagen);
			}
		}			

		if(empty($idclientefinal)){
			$rspta=$clientef->insertar($nombre,$tipo_documento,$num_documento,$telefono,$direccion,$email,$fecha_recepcion,$fecha_entrega,$idlugarrecepcion,$guia,$idestado,$idusuario,$imagen,$_POST["idproducto"],$_POST["cantidad"],$_POST["garantia"],$_POST["observaciones"]);

			echo $rspta ? "Caso servicio tecnico registrado" : "Caso servicio tecnico no se pudieron registrar todos los campos";
		}
		else{
			$rspta=$clientef->editar($idclientefinal,$nombre,$tipo_documento,$num_documento,$telefono,$direccion,$email,$fecha_recepcion,$fecha_entrega,$idlugarrecepcion,$guia,$idestado,$idusuario,$imagen);
	...
	case 'mostrar':
		$rspta=$clientef->mostrar($idclientefinal);
		//Codificamos el resultado utilizando json
		echo json_encode($rspta);
	break;

	case 'eliminar':
		$rspta=$clientef->eliminar($idclientefinal);
		//Codificamos el resultado utilizando json
		echo json_encode($rspta);
	break;
	...
	case 'listarDetalle':
		...
		$rspta = $clientef->listarDetalle($id);
	...
	case 'listar':
	$rspta=$clientef->listar();
	//Vamos a declarar un array
	$data= array();
	while ($reg=$rspta->fetch_object()){

		$data[]=array(
			"0"=>'<button class="btn btn-warning btn-sm" onclick="mostrar('.$reg->idclientefinal.')"><i class="fa fa-pencil" aria-hidden="true"></i></button>'.' <button class="btn btn-danger btn-sm" onclick="eliminar('.$reg->idclientefinal.')"><i class="fa fa-close"></i></button>',
			"1"=>$reg->nombre,
			"2"=>$reg->num_documento,
			"3"=>$reg->telefono,
			"4"=>$reg->direccion,
