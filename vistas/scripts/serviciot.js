...
function init() {
    ...
    $.post("../ajax/serviciot.php?op=selectLugar", function(r) {
        $("#idlugarrecepcion").html(r);
        $("#idlugarrecepcion").selectpicker('refresh');
    });
    //Cargamos  los items del selectLugar
    $.post("../ajax/serviciot.php?op=selectEstado", function(r) {
        $("#idestado").html(r);
...
function listar() {
    tabla = $('#tbllistado').dataTable({
        ...
        "ajax": {
            url: '../ajax/serviciot.php?op=listar',
            type: "get",
            dataType: "json",
            error: function(e) {
...
function listarProductos() {
    tabla = $('#tblproductos').dataTable({
        ...
        buttons: [],
        "ajax": {
            url: '../ajax/serviciot.php?op=listarProductos',
            type: "get",
            dataType: "json",
            error: function(e) {
                console.log(e.responseText);
...
function guardaryeditar(e) {
    ...
    $.ajax({
        url: "../ajax/serviciot.php?op=guardaryeditar",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
...
function mostrar(idserviciotecnico) {
    $.post("../ajax/serviciot.php?op=mostrar", { idserviciotecnico: idserviciotecnico }, function(data, status) {
            ...
            $("#imagenmuestra").show();
            $("#imagenmuestra").attr("src", "../files/serviciotecnico/" + data.imagen);
            $("#imagenactual").val(data.imagen);
            $("#idusuario").val(data.idusuario);
            $("#idserviciotecnico").val(data.idserviciotecnico);
...
function eliminar(idserviciotecnico) {
    bootbox.confirm("¿Esta seguro de eliminar el caso servicio tecnico?", function(result) {
        if (result) {
            $.post("../ajax/serviciot.php?op=eliminar", { idserviciotecnico: idserviciotecnico }, function(e) {
                bootbox.alert(e);
                listar();
            });
        }
