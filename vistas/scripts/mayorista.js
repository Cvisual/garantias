...
//Cargamos  los items del selectLugar
	$.post("../ajax/mayorista.php?op=selectLugar", function(r){
			$("#idlugarrecepcion").html(r);
			$("#idlugarrecepcion").selectpicker('refresh');
	});
//Cargamos  los items del selectLugar
	$.post("../ajax/mayorista.php?op=selectEstado", function(r){
			$("#idestado").html(r);
...
function listar(){
	...
	"ajax":
			{
				url: '../ajax/mayorista.php?op=listar',
				type: "get",
				dataType: "json",
				error: function(e){
...
function listarProductos(){
	tabla=$('#tblproductos').dataTable({
		...
		"ajax":
				{
					url: '../ajax/mayorista.php?op=listarProductos',
					type : "get",
					dataType : "json",						
					error: function(e){
						console.log(e.responseText);	
...
function guardaryeditar(e) {
	...
	$.ajax({
		url: "../ajax/mayorista.php?op=guardaryeditar",
		type: "POST",
		data: formData,		
		contentType: false,
		processData: false,
...
function mostrar(idmayorista) {
	$.post("../ajax/mayorista.php?op=mostrar",{idmayorista : idmayorista}, function(data, status) {
		...
		$("#imagenmuestra").show();
		$("#imagenmuestra").attr("src","../files/mayorista/"+data.imagen);
		$("#imagenactual").val(data.imagen);
		$("#idusuario").val(data.idusuario);
		$("#idmayorista").val(data.idmayorista);
		$("#agregarProducto").hide();	
		$("#btnGuardar").show();		
	})
//Listamos los detalles 
	$.post("../ajax/mayorista.php?op=listarDetalle&id="+idmayorista,function(r){
	        $("#detalles").html(r);
	});
}

//Funcion para eliminar registros
 function eliminar(idmayorista){
 	bootbox.confirm("¿Esta seguro de eliminar el caso mayorista?", function(result){
 		if (result)
 		{
 			$.post("../ajax/mayorista.php?op=eliminar",{idmayorista : idmayorista}, function(e){
 				bootbox.alert(e);
 				listar();
 			});
 		}
